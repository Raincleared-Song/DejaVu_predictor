...
class ActivationRecorder:
    ...
    def record_fp_mlp_query(self, hidden_states, mask_1d):
        ...
        if self.record_flag and self.fp_mlp_id < self.fp_mlp_query.shape[0]:
    ...
    def record_fp_mlp_label(self, up_proj, mask_1d):
    ...
    def record_fp_att_query(self, hidden_states, mask_1d):
        ...
        if self.record_flag and self.fp_att_id < self.fp_att_query.shape[0]:
            ...
            begin, end = self.fp_att_id, min(
                self.fp_att_id + _hidden_states.size(0), self.fp_att_query.shape[0]
            )
    ...
    def record_fp_att_label(self, attn_output, mask_1d):
        ...
        if self.record_flag and self.fp_att_id < self.fp_att_label.shape[0]:
            ...
            begin, end = self.fp_att_id, min(
                self.fp_att_id + attn_output_norm.size(0), self.fp_att_label.shape[0]
            )
            self.fp_att_label[begin:end] = (
...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x, mask_1d):
        ...
        if self.config.pretraining_tp > 1:
            ...
            ACTIVATION_RECORDERS[self.layer_idx].record_fp_mlp_label(up_proj, mask_1d)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        ACTIVATION_RECORDERS[self.layer_idx].record_fp_att_label(attn_output, mask_1d)
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.layer_idx = layer_idx
        if ActivationRecorder.record_flag:
            ACTIVATION_RECORDERS.append(ActivationRecorder(config, layer_idx))
    ...
    def forward(
        ...
        **kwargs,
        ...
        ACTIVATION_RECORDERS[self.layer_idx].record_fp_att_query(hidden_states, mask_1d)
        ...
        ACTIVATION_RECORDERS[self.layer_idx].record_fp_mlp_query(hidden_states, mask_1d)

        hidden_states = self.post_attention_layernorm(hidden_states)
        hidden_states = self.mlp(hidden_states, mask_1d)
        hidden_states = residual + hidden_states
        ...
        if ActivationRecorder.record_flag:
            ACTIVATION_RECORDERS[self.layer_idx].check_id_ok()
